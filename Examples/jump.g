...
//sdl3.SDL_SetHint("SDL_RENDER_DRIVER", "opengl");
//string tmp = sdl3.SDL_GetRenderDriver(1);
//sdl3.SDL_ShowSimpleMessageBox(g.SDL_MESSAGEBOX_INFORMATION, "Message", tmp, null);
...
if (tmpSurface == null) { sdl3.SDL_ShowSimpleMessageBox(g.SDL_MESSAGEBOX_INFORMATION, "Message", "The file cannot be found!", null); return; }
...
SoundtrackerInit("sound/mod/musiklinjen.mod");

RestartGame();
gameStatus = "intro screen";

u64 startTicks = sdl3.SDL_GetTicks();
while (StatusRunning)
{
...
u64 stopTicks = sdl3.SDL_GetTicks();
wavJump.Free();
wavFall.Free();
SoundtrackerFree();
...
//int aantalSeconde = (stopTicks - startTicks) / 1000.0;

//kernel32.SetThreadPriority(thread1Handle, oldThread1Prio);  // Priority of the thread back to the old value.
//sdl3.SDL_ShowSimpleMessageBox(g.SDL_MESSAGEBOX_INFORMATION, "Message", aantalSeconde, null);
//sdl3.SDL_ShowSimpleMessageBox(g.SDL_MESSAGEBOX_INFORMATION, "Message", frameCount + " : " + (aantalSeconde * 60), null);
