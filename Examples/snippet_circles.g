
// This is the CPU version of the circles demo effect.

string pixelWindowTitle = "Circles";        // Inspiration: https://github.com/ivan-guerra/plasma
#include pixelwindow.g

float[SCREEN_WIDTH, SCREEN_HEIGHT] DistanceCache = null;
float[SCREEN_WIDTH, SCREEN_HEIGHT] AngleCache = null;
float time = 0.0;

function Init() {
    DistanceCache = cruntime.calloc(sizeof(float), SCREEN_WIDTH * SCREEN_HEIGHT);
    AngleCache = cruntime.calloc(sizeof(float), SCREEN_WIDTH * SCREEN_HEIGHT);
    for (y in 0..< SCREEN_HEIGHT) {
        float dy = y - SCREEN_HEIGHT_D2;
        for (x in 0..< SCREEN_WIDTH) {
            float dx = x - SCREEN_WIDTH_D2;
            DistanceCache[x,y] = gc.sqrt(dx*dx + dy*dy) * 0.25;
            AngleCache[x,y] = sdl3.SDL_atan2(dy, dx) * 2.0;
        }
    }

#windows GC_CreateThread(Thread2);
#windows GC_CreateThread(Thread3);
#linux GC_Clone(Thread2);
#linux GC_Clone(Thread3);
}

function DeInit() {
    cruntime.free(DistanceCache);
    cruntime.free(AngleCache);

    WaitForThread2();
    WaitForThread3();

    //string showStr = "Best innerloop time: " + debugBestTicks + "ms";
    //sdl3.SDL_ShowSimpleMessageBox(g.SDL_MESSAGEBOX_ERROR, "Message", showStr, null);
    //6ms on Ryzen 7 5700G 3.8 Ghz
}

function Circle(float dist1, float time1, float angle1) : float {
    return sdl3.SDL_sin(dist1 / 4.0 + time1) + sdl3.SDL_sin(angle1 * 2.0 + time1);
}

function DrawPortion(int y) {
    u32* pixelPtr = pixels+(y*SCREEN_LINESIZE);

    for (x in 0..< SCREEN_WIDTH) {
        float pixelDistance = DistanceCache[x,y];
        float pixelAngle = AngleCache[x,y];
        int sinValue = (sdl3.SDL_sin(pixelDistance + time) + sdl3.SDL_sin(pixelAngle + time)) * 128.0;      // Using function is 2ms slower! Circle(pixelDistance, time, pixelAngle) * 128.0;

        /*
        int amountPart = sinValue + 128;
        if (amountPart > 255)
            amountPart = 255;
        if (amountPart < 0)
            amountPart = 0;
        pixels[x,y] = 0xff000000 + (amountPart << 16) + (amountPart << 8) + amountPart;
        */
asm {
  push  rdi rdx
  mov   rdi, [pixelPtr@DrawPortion]
  mov   rax, [sinValue@DrawPortion]
  add   rax, 128
  cmp   rax, 255
  jle   .ceiling255
  mov   rax, 255
.ceiling255:
  cmp   rax, 0
  jge   .ceiling0
  mov   rax, 0
.ceiling0:
  mov   edx, eax
  shl   edx, 8
  or    eax, edx
  shl   edx, 8
  or    eax, edx
  or    eax, 0xff000000
  mov   [rdi], eax
  pop   rdx rdi
}
        pixelPtr = pixelPtr + 4;
    }
}

function Thread2() {
	while (StatusRunning) {
		if (thread2Busy) {
            for (y in SCREEN_HEIGHT_D3 ..< (SCREEN_HEIGHT_D3*2))
                DrawPortion(y);
			thread2Busy = false;
		}
	}
}

function Thread3() {
	while (StatusRunning) {
		if (thread3Busy) {
            for (y in (SCREEN_HEIGHT_D3*2) ..< SCREEN_HEIGHT)
                DrawPortion(y);
			thread3Busy = false;
		}
	}
}


function Update() {
    time = time + 0.10;
    if (time > MATH_2PI)
        time = time - MATH_2PI;

	thread1Busy = StatusRunning;
	thread2Busy = StatusRunning;
	thread3Busy = StatusRunning;

	if (thread1Busy) {
        for (y in 0 ..< SCREEN_HEIGHT_D3)
            DrawPortion(y);
		thread1Busy = false;
	}

    WaitForThread2();
    WaitForThread3();
}
