...
ptr window = sdl3.SDL_CreateWindow("Hexacubes2", SCREEN_WIDTH, SCREEN_HEIGHT, 0);
ptr renderer = sdl3.SDL_CreateRenderer(window, "direct3d"); //null);
ptr texture = sdl3.SDL_CreateTexture(renderer, g.SDL_PIXELFORMAT_ARGB8888, g.SDL_TEXTUREACCESS_STREAMING, 120, 120);
...
function SetVertexTex() {
	...
	for (i in 0 ..< (NR_VERTICES / 3)) {
		...
		verts[index].texY = 0.0;
		verts[index+1].texX = 0.0;
		verts[index+1].texY = 1.0;
		verts[index+2].texX = 1.0;
		verts[index+2].texY = 0.5;
	}
}
SetVertexTex();
...
function SetVertexPosition(int startX, int startY) {
	...
	verts[6].posX = startX + 200;
	verts[6].posY = startY;
	verts[7].posX = startX + 400;
	verts[7].posY = startY + 100;
...
float textureOffsetX = 0.0;
float textureOffsetY = 0.0;
float OffsetYDelta = 0.002;

function UpdateVertexTex() {
	verts[0].texX = 0.0 + textureOffsetX;
	verts[1].texX = 0.0 + textureOffsetX;
	verts[2].texX = 1.0 + textureOffsetX;
	verts[0].texY = 0.0 + textureOffsetY;
	verts[1].texY = 1.0 + textureOffsetY;
	verts[2].texY = 0.5;

	verts[3].texX = 0.0 + textureOffsetX;
	verts[4].texX = 0.0 + textureOffsetX;
	verts[5].texX = 1.0 + textureOffsetX;
	verts[3].texY = 0.0 + textureOffsetY;
	verts[4].texY = 1.0 + textureOffsetY;
	verts[5].texY = 0.5;

	verts[6].texX = 0.0 + textureOffsetX;
	verts[7].texX = 0.0 + textureOffsetX;
	verts[8].texX = 1.0 + textureOffsetX;
	verts[6].texY = 0.0 + textureOffsetY;
	verts[7].texY = 1.0 + textureOffsetY;
	verts[9].texY = 0.5;

	verts[9].texX = 0.0 + textureOffsetX;
	verts[10].texX = 0.0 + textureOffsetX;
	verts[11].texX = 1.0 + textureOffsetX;
	verts[9].texY = 0.0 + textureOffsetY;
	verts[10].texY = 1.0 + textureOffsetY;
	verts[11].texY = 0.5;

	verts[12].texX = 0.0 + textureOffsetX;
	verts[13].texX = 0.0 + textureOffsetX;
	verts[14].texX = 1.0 + textureOffsetX;
	verts[12].texY = 0.0 + textureOffsetY;
	verts[13].texY = 1.0 + textureOffsetY;
	verts[14].texY = 0.5;

	verts[15].texX = 0.0 + textureOffsetX;
	verts[16].texX = 0.0 + textureOffsetX;
	verts[17].texX = 1.0 + textureOffsetX;
	verts[15].texY = 0.0 + textureOffsetY;
	verts[16].texY = 1.0 + textureOffsetY;
	verts[17].texY = 0.5;
}
	...
	for (x in 0..< 120) {
		for (y in 0..19) {
			...
			if (part >= 20) {
				...
				pixels[x, y+20] = 0xff000000;
				pixels[x, y+40] = 0xffffffff;
				pixels[x, y+60] = 0xff000000;
				pixels[x, y+80] = 0xffffffff;
				pixels[x, y+100] = 0xff000000;
			}
			else {
				pixels[x, y] = 0xff000000;
				pixels[x, y+20] = 0xffffffff;
				pixels[x, y+40] = 0xff000000;
				pixels[x, y+60] = 0xffffffff;
				pixels[x, y+80] = 0xff000000;
				pixels[x, y+100] = 0xffffffff;
			}
		}
	}
}
...
while (StatusRunning)
{
	...
	textureOffsetX = textureOffsetX + 0.003;
	//textureOffsetY = textureOffsetY + OffsetYDelta;
	if (textureOffsetX > 1.0)
		textureOffsetX = textureOffsetX - 1.0;
	//if (textureOffsetY > 1.0 or textureOffsetY < 0.0) {
	//	OffsetYDelta = -OffsetYDelta;
	//}
	UpdateVertexTex();
