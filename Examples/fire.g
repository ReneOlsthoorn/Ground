...
coolPixels = cruntime.calloc(1, arraySize);
byte[] fireBufferNew = cruntime.calloc(1, arraySize);
byte[] fireBufferOld = cruntime.calloc(1, arraySize);
...
ptr surface = sdl3_image.IMG_Load("image/groundlogo_960x109.png");
...
SoundtrackerInit("sound/mod/monday - random voice.mod");
...
cruntime.free(coolPixels);
cruntime.free(fireBufferOld);
cruntime.free(fireBufferNew);
