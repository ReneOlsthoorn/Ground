...
int heightOfRoad = (SCREEN_HEIGHT / 8) * 5;
int restHeight = (SCREEN_HEIGHT / 8) * 3;
float space_y = 100.0;
float scale_y = 200.0;
int horizon = 20;
float speed = 0.0;
...
ptr window = sdl3.SDL_CreateWindow("Racer", SCREEN_WIDTH, SCREEN_HEIGHT, 0);
...
class Actor {
	...
	function isHit() : bool {
		...
		float relDistance = this.distance - playerDistance;
		if (relDistance < 56.0 and (sdl3.SDL_fabs(this.displayX - playerPositionDisplay) < 50.0 ))
			return true;
		return false;
	}
}
...
function UpdatePositions() {
	...
	if (sdl3.SDL_fabs(playerSteering) > 0.0002)
...
SoundtrackerInit("sound/mod/bbc shake the world.mod");
